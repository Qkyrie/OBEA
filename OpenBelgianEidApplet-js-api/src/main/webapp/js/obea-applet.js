...
/**
 * Gets the current version of the applet. Use this to be your first method to
 * check if the applet is integrated correctly.
 */
...
/*
 * asynchronous callback functions These should never be called directly,
 * instead, they are used by the applet
 */

/**
 * this method will be called once the applet is loaded, is delegated to
 * onAppletLoaded()
 */
...
/**
 * This method will be called when an eid card is detected, it is delegated to
 * onCardDetected()
 */
...
/**
 * This method is called when the card is removed, it is delegated to
 * onCardRemoved()
 */
...
/**
 * This method is called when the cardReader is removed, it is delegated to
 * onCardReaderRemoved()
 */
function _onCardReaderRemoved() {
	console.log("received from backend: card reader removed");
	if (typeof onCardReaderRemoved === 'function') {
		onCardReaderRemoved();
	} else {
		console.log("[alert] function onCardReaderRemoved does not exist.");
	}
}

/**
 * This method is called when the cardReader is connected, it is delegated to
 * onCardReaderConnected()
 */
function _onCardReaderConnected() {
	console.log("received from backend: card reader connected");
	if (typeof onCardReaderRemoved === 'function') {
		onCardReaderConnected();
	} else {
		console.log("[alert] function onCardReaderConnected does not exist.");
	}
}
...
function verifyPinAsync(_callback) {
	...
	return obea.getExternalAsyncInputDelegate().verifyPin(
			'onPinVerifiedCallback');
}

function signRsaAsync(hash, _callback) {
	onHashNonRepSignedCallback = _callback;
	return obea.getExternalAsyncInputDelegate().signRsa(hash,
			'onHashNonRepSignedCallback');
}

function signRsaAsync(hash, digestAlgorithm, _callback) {
	onHashNonRepSignedCallback = _callback;
	return obea.getExternalAsyncInputDelegate().signRsa(hash, digestAlgorithm,
			'onHashNonRepSignedCallback');
}

function signAuthAsync(hash, _callback) {
	onHashAuthSignedCallback = _callback;
	return obea.getExternalAsyncInputDelegate().signAuth(hash,
			'onHashAuthSignedCallback');
}

function signAuthAsync(hash, digestAlgorithm, _callback) {
	onHashAuthSignedCallback = _callback;
	return obea.getExternalAsyncInputDelegate().signAuth(hash, digestAlgorithm,
			'onHashAuthSignedCallback');
}

function getIdentityAsync(_callback) {
	onGetIdentityCallback = _callback;
	return obea.getExternalAsyncInputDelegate().getIdentity(
			'onGetIdentityCallback');
}
...
function getAddressAsync(_callback) {
	...
	return obea.getExternalAsyncInputDelegate().getAddress(
			'onGetAddressCallback');
}

function getAuthenticationCertificateAsync(_callback) {
	onGetAuthenticationCertificateCallback = _callback;
	return obea.getExternalAsyncInputDelegate().getAuthenticationCertificate(
			'onGetAuthenticationCertificateCallback');
}

function getSigningCertificateAsync(_callback) {
	onSigningCertificateCallback = _callback;
	return obea.getExternalAsyncInputDelegate().getSigningCertificate(
			'onSigningCertificateCallback');
}

function getCACertificateAsync(_callback) {
	onCACertificateCallback = _callback;
	return obea.getExternalAsyncInputDelegate().getCACertificate(
			'onCACertificateCallback');
}

function getRootCACertificateAsync(_callback) {
	onRootCACertificateCallback = _callback;
	return obea.getExternalAsyncInputDelegate().getRootCACertificate(
			'onRootCACertificateCallback');
}

function getRRNCertificateAsync(_callback) {
	onRRNCertificateCallback = _callback;
	return obea.getExternalAsyncInputDelegate().getRRNCertificate(
			'onRRNCertificateCallback');
}
