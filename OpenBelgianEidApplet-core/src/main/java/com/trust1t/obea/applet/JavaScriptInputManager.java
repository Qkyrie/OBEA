 ...
 * 
 * These input methods are called from JavaScript and are delegated to this
 * class. This class is responsible for reading the input and transforming it to
 * an internal call, which then gives an output back to the JavaScript.
 */
public class JavaScriptInputManager implements ExternalInputManager {

	/** The logger. */
	private Logger logger = LoggerFactory
			.getLogger(JavaScriptInputManager.class);
	...
	public ExternalAddress getAddress() {
		if (beidCardController.hasCachedCard()) {
			return AccessController
					.doPrivileged(new PrivilegedAction<JSAddress>() {
						public JSAddress run() {
							try {
								final byte[] idData = beidCardController
										.getCachedCard().readFile(
												FileType.Address);
								final Address address = TlvParser.parse(idData,
										Address.class);
								return new JSAddress(address);
							} catch (InterruptedException ie) {
								return new JSAddress(
										ExternalErrorCodes.CARD_BUSY);
							} catch (Exception e) {
								logger.error(e.getMessage());
								return new JSAddress(
										ExternalErrorCodes.ACCESS_DENIED);
							}
						}
					});
		} else {
	...
	public ExternalPhoto getPhoto() {
		if (beidCardController.hasCachedCard()) {
			return AccessController
					.doPrivileged(new PrivilegedAction<JSPhoto>() {

						public JSPhoto run() {
							try {
								final byte[] photoData = beidCardController
										.getCachedCard().readFile(
												FileType.Photo);
								logger.debug("successfully fetched the photo as bytearray");
								return new JSPhoto(Base64
										.encodeBase64String(photoData));
							} catch (InterruptedException ie) {
	...
	public ExternalIdentity getIdentity() {
		if (beidCardController.hasCachedCard()) {
			return AccessController
					.doPrivileged(new PrivilegedAction<JSIdentity>() {
						public JSIdentity run() {
							try {
								final byte[] idData = beidCardController
										.getCachedCard().readFile(
												FileType.Identity);
								final Identity userIdentity = TlvParser.parse(
										idData, Identity.class);
								return new JSIdentity(userIdentity);
							} catch (InterruptedException ie) {
								return new JSIdentity(
										ExternalErrorCodes.CARD_BUSY);
							} catch (Exception e) {
								logger.error(e.getMessage());
								return new JSIdentity(
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.qkyrie.ts.mkqapplet.external.ExternalInputManager#
	 * getAuthenticationCertificate()
	 */
	public ExternalCertificate getAuthenticationCertificate() {
		if (beidCardController.hasCachedCard()) {
			return AccessController
					.doPrivileged(new PrivilegedAction<ExternalCertificate>() {
						public ExternalCertificate run() {
							try {
								final String authenticationCertificate;
								authenticationCertificate = Base64
										.encodeBase64String(beidCardController
												.getCachedCard()
												.getAuthenticationCertificate()
												.getEncoded());
								logger.debug("authenticationCertificate: {}",
										authenticationCertificate);
								return new JSCertificate(
										authenticationCertificate,
										ExternalErrorCodes.SUCCEEDED);
								...
								return new JSCertificate(
										ExternalErrorCodes.CERTIFICATE_PROBLEM);
	 ...
	 * 
	 * @see
	 * com.qkyrie.ts.mkqapplet.external.ExternalInputManager#getSigningCertificate
	 * ()
	 */
							...
							try {
								...
								signingCertificate = Base64
										.encodeBase64String(beidCardController
												...
												.getEncoded());
								logger.debug("signingCertificate: {}",
										signingCertificate);
								return new JSCertificate(signingCertificate,
										ExternalErrorCodes.SUCCEEDED);
							} catch (CardException e) {
								return new JSCertificate(
										ExternalErrorCodes.CARD_BUSY);
	 ...
	 * 
	 * @see
	 * com.qkyrie.ts.mkqapplet.external.ExternalInputManager#getCACertificate()
	 */
					...
					.doPrivileged(new PrivilegedAction<ExternalCertificate>() {
						public ExternalCertificate run() {
							try {
								...
								caCertificate = Base64
										.encodeBase64String(beidCardController
												.getCachedCard()
												.getCACertificate()
												.getEncoded());
								logger.debug("caCertificate: {}", caCertificate);
								return new JSCertificate(caCertificate,
										ExternalErrorCodes.SUCCEEDED);
							} catch (CardException e) {
								return new JSCertificate(
										ExternalErrorCodes.CARD_BUSY);
	 ...
	 * 
	 * @see
	 * com.qkyrie.ts.mkqapplet.external.ExternalInputManager#getRootCACertificate
	 * ()
	 */
					...
					.doPrivileged(new PrivilegedAction<ExternalCertificate>() {
						public ExternalCertificate run() {
							try {
								...
								rootCACertificate = Base64
										.encodeBase64String(beidCardController
												.getCachedCard()
												.getRootCACertificate()
												.getEncoded());
								logger.debug("rootCACertificate: {}",
										rootCACertificate);
								return new JSCertificate(rootCACertificate,
										ExternalErrorCodes.SUCCEEDED);
							} catch (CardException e) {
								return new JSCertificate(
										ExternalErrorCodes.CARD_BUSY);
	 ...
	 * 
	 * @see
	 * com.qkyrie.ts.mkqapplet.external.ExternalInputManager#getRRNCertificate()
	 */
					...
					.doPrivileged(new PrivilegedAction<ExternalCertificate>() {
						public ExternalCertificate run() {
							try {
								...
								rrnCertificate = Base64
										.encodeBase64String(beidCardController
												.getCachedCard()
												.getAuthenticationCertificate()
												.getEncoded());
								logger.debug("rrnCertificate: {}",
										rrnCertificate);
								return new JSCertificate(rrnCertificate,
										ExternalErrorCodes.SUCCEEDED);
							} catch (CardException e) {
								return new JSCertificate(
										ExternalErrorCodes.CARD_BUSY);
	...
	public ExternalCertificateChain getAuthenticationCertificateChain() {
		if (beidCardController.hasCachedCard()) {
			return AccessController
					.doPrivileged(new PrivilegedAction<ExternalCertificateChain>() {
						public ExternalCertificateChain run() {
							try {
								List<String> authenticationCertificateChain = new LinkedList<String>();
								for (X509Certificate x509Certificate : beidCardController
										.getCachedCard()
										.getAuthenticationCertificateChain()) {
									final String authenticationChainPart = Base64
											.encodeBase64String(x509Certificate
													.getEncoded());
									authenticationCertificateChain
											.add(authenticationChainPart);
									logger.debug("authenticationChainPart: {}",
											authenticationChainPart);
								}

								return new JSCertificateChain(
										authenticationCertificateChain);
							} catch (CardException e) {
								return new JSCertificateChain(
										ExternalErrorCodes.CARD_BUSY);
						...
						public ExternalCertificateChain run() {
							try {
								...
								for (X509Certificate x509Certificate : beidCardController
										.getCachedCard()
										.getSigningCertificateChain()) {
									final String signingChainPart = Base64
											.encodeBase64String(x509Certificate
													.getEncoded());
									signingCertificateChain
											.add(signingChainPart);
									logger.debug("signingChainPart: {}",
											signingChainPart);
								}
	 ...
	 * 
	 * @see
	 * com.qkyrie.ts.mkqapplet.external.ExternalInputManager#getCACertificateChain
	 * ()
	 */
						...
						public ExternalCertificateChain run() {
							try {
								...
								for (X509Certificate x509Certificate : beidCardController
									...
									final String caCertificateChainPart = Base64
											.encodeBase64String(x509Certificate
													.getEncoded());
									caCertificateChain
											.add(caCertificateChainPart);
									logger.debug("caCertificateChainPart: {}",
											caCertificateChainPart);
								}
	 ...
	 * 
	 * @see
	 * com.qkyrie.ts.mkqapplet.external.ExternalInputManager#getRRNCertificateChain
	 * ()
	 */
						...
						public ExternalCertificateChain run() {
							try {
								...
								for (X509Certificate x509Certificate : beidCardController
									...
									final String rrnCertificateChainPart = Base64
											.encodeBase64String(x509Certificate
													.getEncoded());
									rrnCertificateChain
											.add(rrnCertificateChainPart);
									logger.debug("rrnCertificateChainPart: {}",
											rrnCertificateChainPart);
								}
	...
	public boolean verifyPin() {
		...
		if (response.getErrorCode().equals(
				ExternalErrorCodes.SUCCEEDED.getErrorCode())) {
	...
	public ExternalSignResponse signRsa(final String base64EncodedBytes,
		...
		if (beidCardController.hasCachedCard()) {
			return AccessController
					.doPrivileged(new PrivilegedAction<ExternalSignResponse>() {
						public ExternalSignResponse run() {
							try {
								byte[] originalBytes = null;
								originalBytes = Base64
								...
								String returnValueEncoded = Base64
										.encodeBase64String(returnValue);
								...
								return new JSSignResponse(
										ExternalErrorCodes.ILLEGAL_ARGUMENT);
							}
						}
	 ...
	 * 
	 * @see
	 * com.trust1t.obea.external.ExternalInputManager#setPin(java.lang.String)
	 */
